...
const App: React.FC = () => {
  ...
  const yearlySummary = useMemo(() => {
    const y = summaryYear;
    if (!y) return [] as { month: string; sales: number; expenses: number; profit: number }[];

    const result: { month: string; sales: number; expenses: number; profit: number }[] = [];
    for (let m = 1; m <= 12; m++) {
  ...
  async function downloadDailyExcelWithTemplate(
    ...
    // 템플릿 파일은 public/daily-template.xlsx 에 위치해야 함
    const res = await fetch("/daily-template.xlsx");
    if (!res.ok) {
      alert("엑셀 템플릿을 불러오지 못했습니다. daily-template.xlsx 위치를 확인하세요.");
      return;
    }
      ...
      <div className="app-shell">
        ...
        <header className="app-header">
          <div className="header-left">
            <img src="/더바름진 고화질.png" className="logo-img" alt="GENE SPORTS CLINIC" />
